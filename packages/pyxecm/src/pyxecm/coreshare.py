...
class CoreShare:
    ...
    def search_users(
        ...
        request_header = self.request_header_admin()
        request_url = (
            self.config()["searchUserUrl"] + f"/{user_status}" + "?q=" + query_string + "&pageSize=" + str(page_size)
        )
    ...
    def revoke_group_share(self, group_id: str, resource_id: str) -> dict | None:
        ...
        request_url = self.config()["foldersUrlv1"] + f"/{resource_id}" + "/collaboratorsAsAdmin/" + str(group_id)
